...
struct zip_t *zip_open(int level, int append) {
    ...
    if (zip) {
        ...
        if (!mz_zip_writer_init_heap(&(zip->archive), 0, MAX_ZIPMEM)) {
            cleanup(zip);
            return NULL;
        }
    }
...
void zip_close(struct zip_t *zip, void **pBuf, size_t *size) {
    if (zip) {       		
		// Always finalize, even if adding failed for some reason, so we have a valid central directory.
		mz_zip_writer_finalize_heap_archive(&(zip->archive), pBuf, size);
